 ...
 * partnerId === null means the Wolf went Lone Wolf.
...
export function wolfHolePoints(
  ...
  if (partnerId) {
    ...
    if (teamBest < oppBest) {
      ...
      for (const id of others) points[id] = 3;
    }
    // tie -> everyone stays at 0
  } else {
 ...
 * partner chosen (golfer_id), null for Lone Wolf, or undefined if the
 * Wolf hasn't decided yet for that hole (that hole contributes no points
 * until a decision is recorded).
 */
export function wolfRoundPoints(
  holeNumbers: number[],
  playerIdsInTeeOrder: string[],
  decisions: Record<number, string | null | undefined>,
  netByGolfer: Record<string, Record<number, number>>
) {
